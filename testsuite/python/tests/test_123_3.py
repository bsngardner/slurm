...
def test_overlap_weeks_reverse(request, get_and_down_nodes, reocurring_flag):
    """Verify that reoccurring reservations don't overlap nodes if they are have a start time difference greater than a week"""
...
def test_overlap_reocurring(request, get_and_down_nodes, reocurring_flag):
    """Verify that reoccurring reservations don't overlap when the start at 1 reoccurring period"""
...
def test_overlap_reocurring_week(request, get_and_down_nodes, reocurring_flag):
    """Verify that reoccurring reservations don't overlap if they are have a start time difference greater than a week"""
...
def test_overlap_replacing(request, get_and_down_nodes, reocurring_flag):
    ...
    logging.info(
        "Waiting until resv1 and resv2 are ACTIVE (should be started in 1 minute)..."
    )
    atf.repeat_until(
    ...
    logging.info(
        "Ensuring that there resv2 won't steal any node (it will try it every PERIODIC_TIMEOUT=30s):"
    )
...
def test_overlap_weekdays(request, get_and_down_nodes, reocurring_flag, week_flag):
    """Verify that reoccurring reservations with weekdays and weekends handle overlapping properly with other reocurrings"""
...
def test_no_overlap_weekday_weekend(
    ...
    """Verify that reoccurring reservations with weekdays and weekends handle overlapping properly between them"""
