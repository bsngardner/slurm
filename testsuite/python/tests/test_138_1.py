...
def test_stdin_broadcast(input_file, tmp_path):
    ...
    for i in range(0, iteration_count):
        ...
        exit_code = atf.run_command_exit(
            f"srun --input={input_file} --output={output_file} --error=/dev/null -n {task_count} -N {node_count} -t 1 cat -"
        )
        ...
        assert (
            out_size == in_size * task_count
        ), f"output file size ({out_size}) was not {task_count} times the input file size ({in_size})"
